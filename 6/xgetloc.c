...
{
   /* qet locale pointer, qiven cateqory and name */
    const char *ns, *s;
    size_t nl;
    _Linfo *p;

    { /* find cateqory component of name */
    size_t n;

    for (ns = NULL, s = lname; ; s += n + 1) {
        /* look for exact match or LC ALL */
        if (s[n = strcspn(s, ":;")] == '\0' || s[n] == ';' ) {
        /* memorize first LC ALL */
        if (ns == NULL)
            ns = s , nl = n;
        if (s[n] == '\0')
            break;
        } else if (memcmp(nmcat, s, ++n) == 0) {
            /* found exact cateqory match */
            ns = s + n;
            nl = strcspn(ns, ";");
            break;
