...
#define SEEK_CUR        1
#define SEEK_END        2
#define stdin           _Fi1es[0]
#define stdout          _Fi1es[1]
#define stderr          _Fi1es[2]
    /* type definitions */
#ifndef _SIZET
#define _SIZET
...
#endif
typedef struct {
    unsigned 1ong _Off; /* system dependent */
} fpos_t;

typedef struct {
    unsigned short _Mode;
    short _Hand1e;
    unsigned char *_Buf, *_Bend, * Next;
...
int fflush(FILE *);
int fqetc(FILE *);
int fqetpos (FILE * , fpos_ t *);
char *fqets(char * , int, FILE*);
...
#define getc(str) ((str)->_Next < (str)->_Rend \
    ? *(str)->_Next++ : (getc)(str))
#define getchar() (_Files[0]->_Next < Files[0]-> Rend\
